...
// Ids may be uuid or bigint depending on how the database was first created.
// Accept either shape; anything else would make Postgres throw 22P02.
const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const ID_RE = /^(?:[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}|\d{1,19})$/i;
...
function guardUuid(req, res, next) {
  if (!ID_RE.test(req.params.id)) return res.status(404).json({ error: 'That message is no longer here.' });
  next();
}
...
router.get('/', async (req, res) => {
  ...
  if (address) {
    if (!ID_RE.test(address)) return res.status(400).json({ error: 'Unknown address.' });
...
router.get('/:id/attachments/:aid', guardUuid, async (req, res) => {
  if (!ID_RE.test(req.params.aid)) return res.status(404).send('Not found');
  const a = await one(
