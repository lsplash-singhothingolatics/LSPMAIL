...
router.post('/send', async (req, res) => {
  ...
  try {
    provider = await sendMail({
      ...
      to: recipients, cc, subject, text: bodyText, html: bodyHtml, replyTo: addr.address, attachments,
    });
  } catch (e) {
  ...
  const msg = await tx(async (c) => {
    const m = (await c.query(
      ...
      [req.user.id, addr.id, addr.address, addr.label, recipients, cc, subject,
